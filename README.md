...
Problem definition:
Teachers manually calculate totals, averages, and grades for each student, which is slow and error‑prone. This program automatically calculates the average percentage and final grade from marks entered for any number of subjects.
...
Input marks for each of the subjects respectively.

Compute total and average percentage.

Assign a letter grade (A–F) based on the average.

Display the student’s name, number of subjects, total, average, and grade.

Top-down design / modularization
~Main steps:

Read inputs (name, number of subjects, marks).

Process data (sum marks, compute average, decide grade).
...
Test with different values of n (e.g., 1, 3, 5).

Try high averages (e.g., 95) to check grade A and low ones (e.g., fifty‑sixty) to check other grades.

Test invalid inputs (like text instead of numbers) to see where future improvements (validation) could be added.
